...
function addListeners(map) {
  d3.select('#filterCheese').on('click', function(d){
    filterCheese(map);
  });
  d3.select('#meatFilter').on('click', function(d){
    filterMeat(map);
  });
...
function filterCheese(map) {
  var cheeseBombs = bombs.filter(function(d) {
    return d.Cheese == 'Y';
  });
  mapBubbles(map, cheeseBombs);
}

function filterMeat(map) {
  var meatBombs = bombs.filter(function(d) {
    return d.Meat == 'Y';
  });
  mapBubbles(map, meatBombs);
}

function clearFilters(map) {
  var clearBombs = bombs.filter(function(d) {
    return true;
  });
  mapBubbles(map, clearBombs);
}